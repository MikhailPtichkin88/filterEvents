...
const EventCards = () => {
    ...
    if (filters.showFirst === "oldest") {
        ...
        filteredEvents.sort((a, b) => Date.parse(a.date) - Date.parse(b.date)).reverse()
    }

    let filterResult = [...filteredEvents]
    if (filters.searchText.trim().length >= 3) {
        filterResult = filteredEvents.filter(el=>el.title.toLowerCase().includes(filters.searchText.trim().toLowerCase()))
